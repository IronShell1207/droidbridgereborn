namespace CustomControlsLib.Controls
{
	...
	public sealed class OutlinedTextBlock : UserControl
	{
		...
		public static readonly DependencyProperty CasingProperty = DependencyProperty.Register(
			"Casing", typeof(CharacterCasing), typeof(OutlinedTextBlock), new PropertyMetadata(default(CharacterCasing), OnPropertyChanged));
		...
		private void OutlinedTextBlock_Loaded(object sender, RoutedEventArgs e)
		{
			...
			_tokenFontSize = this.RegisterPropertyChangedCallback(CasingProperty, OnPropertyChanged2);
		...
		private CanvasTextLayout CreateTextLayout(ICanvasResourceCreator resourceCreator, Size size)
		{
			...
			string text = Casing switch
			{
				CharacterCasing.Lower => this.Text.ToLower(),
				CharacterCasing.Normal => this.Text,
				CharacterCasing.Upper => this.Text.ToUpper()
			};
